...
function checkState() {
  ...
  const getSurfaceDistance = (location) => {
    return this.SURFACE_DISTANCE_PER_ONE_DEGREE[parseInt(location.latitude / BETWEEN_DEGREE)]; //depend on latitude
  }

  const getLatitudeDistance = (location) => {
    return this.getSurfaceDistance(location).latitude * this.THOUSAND_METER;
  }

  const getLongitudeDistance = (location) => {
